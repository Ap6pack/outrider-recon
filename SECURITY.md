...
The browser may inventory `finding_candidate` metadata, explicitly promote a reviewed candidate into an append-only local finding, and verify finding provenance point-in-time. It still provides no automatic promotion, finding edit/delete, reports, exports, execution, arbitrary MCP, recon, result upload, or artifact transfer.
