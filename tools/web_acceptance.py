...
def run_acceptance() -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix='outrider-web-acceptance-') as td:
        ...
        disabled_client=TestClient(create_app(root, control_token=CONTROL_VALUE, enrichment_executor=fake))
        ...
        def s_onboarding():
            ...
            r=disabled_client.post('/api/runs',json={'target':'example.com','actor':'authorized-operator','authorization_reference':'opaque authorization reference','engagement_platform':'HackerOne','traffic_header':{'name':'X-Bug-Bounty','value':'H1-authorized-operator'},'in_scope':['example.com'],'out_of_scope':[],'confirmed':True},headers=h)
            ...
            inv=disabled_client.get('/api/runs').json(); assert_ok(inv['total']==1 and inv['runs'][0]['next_action']=='Review Scope','resume inventory')
        ...
        def s_guided():
            rid=ctx['onboard_id']; run_dir=ctx['onboard_dir']
            before=files(run_dir)
            g=disabled_client.get(f"/api/runs/{rid}/guide")
            assert_ok(g.status_code==200,g.text); guide=g.json()
            assert_ok(guide['phase']['label']=='Setup','phase setup')
            assert_ok(any(p['label']=='Scope' and p['status']=='current' for p in guide['progress']),'scope current')
            assert_ok(guide['next_action']['label']=='Review Scope','review scope')
            assert_ok(files(run_dir)==before,'guide read-only')
            scope=disabled_client.get(f"/api/runs/{rid}/scope").json(); assert_ok(scope['valid'] is True,'scope review loads')
            old_rev=guide['guide_revision']; old_bytes=(run_dir/'run.jsonl').read_bytes(); old_count=len(old_bytes.splitlines())
            confirm=disabled_client.post(f"/api/runs/{rid}/guide/actions/confirm_scope",json={'expected_state':guide['current_state'],'expected_guide_revision':guide['guide_revision'],'actor':'authorized-operator','reason':None,'confirmed':True},headers=h)
            assert_ok(confirm.status_code==200,confirm.text)
            assert_ok(load_manifest(run_dir).run_id==rid and len((run_dir/'run.jsonl').read_bytes().splitlines())==old_count+1,'one transition appended')
            assert_ok(confirm.json()['guide']['current_state']=='scoped','scoped')
            assert_ok(fake.http_calls==[] and fake.dns_calls==[],'no discovery ran')
            assert_ok(not any((run_dir/'artifacts').iterdir()) and (run_dir/'evidence.jsonl').read_text()=='' ,'no artifacts or evidence')
            guide2=disabled_client.get(f"/api/runs/{rid}/guide").json(); assert_ok(guide2['next_action']['label']=='Begin Discovery','begin discovery')
            before2=files(run_dir); count2=len((run_dir/'run.jsonl').read_bytes().splitlines())
            begin=disabled_client.post(f"/api/runs/{rid}/guide/actions/begin_discovery",json={'expected_state':guide2['current_state'],'expected_guide_revision':guide2['guide_revision'],'actor':'authorized-operator','reason':None,'confirmed':True},headers=h)
            assert_ok(begin.status_code==200,begin.text); assert_ok(begin.json()['guide']['current_state']=='collecting','collecting')
            assert_ok(len((run_dir/'run.jsonl').read_bytes().splitlines())==count2+1,'one additional transition')
            assert_ok(fake.http_calls==[] and fake.dns_calls==[],'no provider call')
            assert_ok((run_dir/'evidence.jsonl').read_text()=='' and (run_dir/'approvals.jsonl').read_text()=='' and not list((run_dir/'contracts'/'requests').glob('*.json')) and not list((run_dir/'contracts'/'results').glob('*.json')) and (run_dir/'findings.jsonl').read_text()=='' and (run_dir/'report.md').read_bytes()==before2['report.md'],'no evidence approval contract finding report')
            guide3=disabled_client.get(f"/api/runs/{rid}/guide").json(); assert_ok(guide3['next_action']['label']=='Set Up Discovery','disabled guidance')
            stale=disabled_client.post(f"/api/runs/{rid}/guide/actions/begin_discovery",json={'expected_state':'scoped','expected_guide_revision':old_rev,'actor':'authorized-operator','reason':None,'confirmed':True},headers=h)
            assert_ok(stale.status_code==409,'stale rejected')
            assert_ok(files(run_dir)==files(run_dir),'file bytes preserved after stale')
            return 'guided scope confirmation and Begin Discovery verified without execution or artifact creation'
        stage('guided workflow', s_guided)
        def s2():
