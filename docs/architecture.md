...
The contract layer does not implement automatic skill execution, automatic evidence capture, concurrent orchestration, authenticated actors.
...
The optional local web control plane provides run overview, scope review, state review, evidence review, approval review, contract review, finding review, integrity status, guarded atomic run creation, initialized-only scope replacement, browser scope checks, guarded workflow-state transitions, guarded approval grants/revocations, action-policy checks, metadata-only artifact inventory, guarded existing-artifact evidence registration, and point-in-time evidence verification. It is implemented as a framework-independent projection layer plus an optional FastAPI/static UI layer. Mutations resolve runs by UUID beneath the configured runs root where applicable, require a process-local control token plus same-origin checks, and serialize through a process-local lock. Run creation derives safe server-side directory names and builds complete runs in temporary siblings before atomic rename. Scope replacement uses an exact-file SHA-256 stale-write precondition, is allowed only while initialized, preserves non-scope top-level YAML values, and writes `scope.yaml` atomically. State transitions continue to delegate policy and append-only events to `outrider.state`. Approval controls delegate to `outrider.approval`, use an exact `approvals.jsonl` SHA-256 stale-write token, and do not execute evaluated or approved actions.
...
## Governed agent-to-agent loop

`outrider.orchestrator` drives a run through repeated request/result hops without adding authority. Each hop is created only through `outrider.skill_contract.create_skill_request`, which refuses when current scope, approval, evidence, or workflow state disallow it; results are consumed only after `validate_skill_result` returns valid; and next hops are derived only from validated `recommended_actions`. Skill execution is delegated to an injected executor (`outrider.executors`): a deterministic stub for tests and the benchmark, or a gated out-of-process agent executor for offline or live use that is disabled by default. Auto-dispatch is capped at local and passive actions; active enumeration requires a standing approval and an allowed state, checked per hop; intrusive and prohibited actions are handoff-only. The loop is bounded, resumable over the existing contract inventory, halts on external state change, and never promotes findings. See ADR 0017.

`outrider.verifier` runs an independent, advisory pass over finding candidates, attempting to falsify each against its cited evidence and current scope and recording `supported`, `refuted`, or `insufficient_evidence` verdicts under `contracts/verification/`. Verdicts never promote, never delete, and never change promotion eligibility; the human reviewer remains the decision maker. See ADR 0018.

`outrider.benchmark` builds synthetic run folders from a `ground-truth-v1` corpus, drives the orchestrator with the stub executor, and scores schema conformance, discovered-candidate precision/recall, and finding-candidate coverage with a deterministic judge. It runs with no model and no network, so corpus metrics are reproducible, and it confirms the governed loop promotes nothing. An optional model-assisted judge is offline-only. See ADR 0019.
