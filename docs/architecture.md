...
The optional local web control plane provides run overview, scope review, state review, evidence review, approval review, contract review, finding review, integrity status, and one guarded mutation for workflow-state transitions on existing run folders. It is implemented as a framework-independent projection layer plus an optional FastAPI/static UI layer. State mutations resolve runs by UUID beneath the configured runs root, require a process-local control token plus same-origin checks, serialize through a process-local lock, enforce an expected-current-state precondition, and delegate transition policy and append-only events to `outrider.state`.
