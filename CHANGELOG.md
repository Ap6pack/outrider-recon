...
- The Import form additionally prefills from the target folder's `memory.md` when present (`bridge.parse_target_memory` via `GET /api/import/suggest`): it maps the labelled header (`**Target:**`, `**Program:**`, `**Researcher:**`, `**Main Scope URL:**`) to target/platform/operator/authorization, and suggests in-scope **only** from the primary `## In-Scope Assets` table — never sweeping in nuanced "related domains" tables. Everything is prefill-for-review: the operator still edits and confirms scope + authorization. The parse reads only `memory.md`, never raises, and returns only the fields it finds. An optional engagement-platform selector was added to the form. In-scope URL assets (e.g. `https://www.example.com/book/`) are reduced to their host (`www.example.com`) and deduped, because the governed scope model accepts hosts/domains/IPs/CIDRs rather than URLs with paths; the suggested target is kept consistent with those hosts so the prefill imports without a "target is not allowed by scope" error.
