...
- Added self-healing to the governed orchestrator loop so an unattended run recovers on its own (ADR 0022). A transient executor failure (timeout, surfaced as the new `TransientExecutorError`) is retried with backoff before the loop gives up; a permanent failure or invalid result triggers a bounded, deduped passive fallback (`public_source_lookup`) on the same candidate that never escalates action class; and the loop is crash-safe — it writes a `contracts/inflight/<id>` marker before each hop, then on resume re-runs interrupted local/passive hops while deferring an interrupted active/intrusive hop for human review instead of re-firing it. Tunable via `outrider orchestrate run --max-transient-retries/--retry-backoff-seconds/--no-replan`.
...
- The Import form additionally prefills from the target folder's `memory.md` when present (`bridge.parse_target_memory` via `GET /api/import/suggest`): it maps the labelled header (`**Target:**`, `**Program:**`, `**Researcher:**`, `**Main Scope URL:**`) to target/platform/operator/authorization, and suggests in-scope **only** from the primary `## In-Scope Assets` table — never sweeping in nuanced "related domains" tables. Everything is prefill-for-review: the operator still edits and confirms scope + authorization. The parse reads only `memory.md`, never raises, and returns only the fields it finds. An optional engagement-platform selector was added to the form. In-scope URL assets (e.g. `https://www.example.com/book/`) are emitted as scheme-agnostic `host[/path]` scope rules (see the URL/path scope support below), preserving the exact path scope instead of reducing to the bare host; the suggested target reduces to the primary in-scope host so the prefill imports without a "target is not allowed by scope" error.
...
- Fixed a `SyntaxError` in `mcp-server/server.py` where `hudsonrock_lookup` was left with an orphaned docstring tail and dead request body after being converted to delegate through the shared policy-gated executor; the module now parses and imports, restoring `tests/test_mcp_server`.
