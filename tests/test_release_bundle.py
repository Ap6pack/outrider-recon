...
class ReleaseBundleTests(unittest.TestCase):
    ...
    def test_bundle_contents_manifest_and_determinism(self):
        with tempfile.TemporaryDirectory() as td1, tempfile.TemporaryDirectory() as td2:
            ...
            archive=Path(td1)/'outrider-recon-bundle-4.0.0.zip'
            self.assertTrue(archive.exists())
            with zipfile.ZipFile(archive) as z:
                names=z.namelist()
                self.assertTrue(all(n.startswith('outrider-recon-bundle-4.0.0/') for n in names))
                ...
                for required in ['README.md','.claude-plugin/plugin.json','pyproject.toml','outrider/skill_catalog.json','contracts/skill-request-v1.schema.json','skills/_shared/run-contract.md','tools/build_release_bundle.py']:
                    self.assertIn('outrider-recon-bundle-4.0.0/'+required, names)
                ...
                manifest=json.loads(z.read('outrider-recon-bundle-4.0.0/RELEASE-MANIFEST.json'))
                ...
                for entry in manifest['files'][:25]:
                    data=z.read('outrider-recon-bundle-4.0.0/'+entry['path'])
    ...
    def test_output_refusal_force_and_json(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(data['bundle_filename'],'outrider-recon-bundle-4.0.0.zip')
